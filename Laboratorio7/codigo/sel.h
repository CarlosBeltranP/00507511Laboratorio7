...
Matrix createLocalK(int e,mesh &m){
    ...
    // [ A+K  G+X ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixG,matrixD,matrixX;
    ...
    productRealMatrix((tau*J)/(4*6*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixA);
    ...
    productRealMatrix(-(delta*Ae)/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixX);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixG,matrixX,6,3));
